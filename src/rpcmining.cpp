...
//    Litecoin and modified for Tengri.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated Tengri network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for Tengri
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the Tengri network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Tengri is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Tengri is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Tengri is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Tengri is downloading blocks...");
